...
func _build_hat() -> void:
	...
	# Ngôi sao vàng trên gù
	MeshBuilder.box(head, Vector3(0,  0.40,  0.22), Vector3(0.07, 0.07, 0.02), _star)
	MeshBuilder.box(head, Vector3(0,  0.36,  0.22), Vector3(0.03, 0.12, 0.02), _star)
	# Vành mũ nhỏ sau đầu
...
func _build_arms() -> void:
	...
	for arm in [arm_l, arm_r]:
		...
		# Cổ tay + bàn tay da
		MeshBuilder.box(arm, Vector3(0, -0.22, 0), Vector3(0.12, 0.06, 0.12), _sk)
		MeshBuilder.box(arm, Vector3(0, -0.28, 0.01), Vector3(0.12, 0.07, 0.10), _sk)
	# Weapon pivot
...
func _build_legs() -> void:
	...
	for leg in [leg_l, leg_r]:
		...
		MeshBuilder.box(leg, Vector3(0, -0.25, 0.08), Vector3(0.08, 0.02, 0.02), _shod)
