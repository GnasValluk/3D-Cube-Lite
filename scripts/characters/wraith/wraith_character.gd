## wraith/wraith_character.gd
## Sinh vật thù địch: Bóng Đêm (Night Wraith) — đồng bằng.
## Hồn ma lơ lửng giữ khoảng cách, bắn tia tối (ShadowBolt) từ xa, không
## tiếp cận cận chiến. Chết drop Wraith Tear. Chỉ gặp về đêm.

extends CharacterBase
class_name WraithCharacter

const HP: float = 22.0
const ATK: int = 6
const SPEED: float = 2.0
const SCALE: float = 1.0

const AGGRO_RANGE: float = 20.0
const FIRE_RANGE: float = 14.0
const MIN_KEEP_DIST: float = 3.5
const FIRE_COOLDOWN: float = 2.2
const WANDER_RADIUS: float = 7.0
const WANDER_SPEED_MULT: float = 0.35

const _DroppedItem = preload("res://scripts/items/entities/dropped_item.gd")
const _ExpOrb = preload("res://scripts/items/entities/experience_orb.gd")
const _ShadowBolt = preload("res://scripts/characters/wraith/shadow_bolt.gd")
const EXP_DROP_RATE: float = 0.32

var _mesh: WraithMesh
var _home: Vector3 = Vector3.ZERO
var _player: Node3D = null
var _target_dir: Vector3 = Vector3.FORWARD
var _base_move_speed: float = 2.0
var _fire_cd: float = 0.0
var _wander_t: float = 0.0
var _idle_t: float = 0.0
var _charging: bool = false
var _hover_phase: float = 0.0

func _init() -> void:
	show_world_hp_bar = true

func _build_character() -> void:
	_is_player = false
	character_name = "Bóng Đêm"
	mob_bonus_lv = 3
	level = compute_level()
	var smult: float = get_stat_mult()
	max_hp = maxi(1, int(HP * smult))
	hp = max_hp
	_base_move_speed = SPEED * smult
	move_speed = _base_move_speed
	sprint_speed = move_speed
	acceleration = 22.0
	friction = 10.0
	defense = 1
	attack_power = ATK
	jump_height = 0.5
	jump_time_rise = 0.22
	jump_time_fall = 0.16
	melee_range = 0.4
	hit_radius = 0.34
	_world_hp_enabled = true

	var sc: float = SCALE
	scale = Vector3(sc, sc, sc)

	var col := CollisionShape3D.new()
	var cap := CapsuleShape3D.new()
	cap.radius = 0.30
	cap.height = 1.10
	col.shape = cap
	col.position = Vector3(0, 0.62, 0)
	add_child(col)

	# Lơ lửng nhẹ (visual): rig trôi lên-xuống; body vẫn đi như quái thường
	move_speed = _base_move_speed

	_mesh = WraithMesh.new()
	_mesh.build(self)
	_rig = _mesh.rig

func _ready() -> void:
	super._ready()
	_home = global_position
	add_to_group("wraith")
	_target_dir = -global_transform.basis.z
	_wander_t = randf_range(1.0, 2.5)

func _find_player() -> Node3D:
	var world := get_tree().current_scene
	if world == null:
		return null
	var mgr := world.get_node_or_null("CharacterManager") as CharacterManager
	if mgr:
		return mgr.get_current_character()
	return null

func _read_input() -> Vector3:
	return _target_dir

func _animate(delta: float) -> void:
	if not is_alive:
		return
	_fire_cd = max(_fire_cd - delta, 0.0)
	_hover_phase += delta
	# Trôi lơ lửng nhẹ nhàng (visual)
	if _rig:
		_rig.position.y = sin(_hover_phase * 1.2) * 0.06

	if _player == null or not is_instance_valid(_player):
		_player = _find_player()

	var chasing := false
	var dist := 0.0
	if _player and is_instance_valid(_player) and _player.get("is_alive"):
		var to_player := _player.global_position - global_position
		if abs(to_player.y) <= 4.0:
			to_player.y = 0.0
			dist = to_player.length()
			chasing = dist < AGGRO_RANGE
			if chasing:
				if dist > MIN_KEEP_DIST:
					_target_dir = to_player.normalized()
				elif dist > 0.01:
					_target_dir = -to_player.normalized()
				if dist <= FIRE_RANGE and _fire_cd <= 0.0:
					_fire_cd = FIRE_COOLDOWN
					_fire_bolt(to_player)
					_charging = true
				else:
					_charging = false
			_charging = _charging and dist <= FIRE_RANGE and _fire_cd > FIRE_COOLDOWN - 0.2 and _fire_cd < FIRE_COOLDOWN

	move_speed = _base_move_speed * (WANDER_SPEED_MULT if not chasing else 0.8)
	sprint_speed = move_speed

	if not chasing:
		var to_home := _home - global_position
		to_home.y = 0.0
		var home_dist := to_home.length()
		if home_dist > WANDER_RADIUS:
			_target_dir = to_home.normalized()
			_idle_t = 0.0
			_wander_t = randf_range(0.5, 1.5)
		elif _idle_t > 0.0:
			_idle_t -= delta
			_target_dir = _target_dir.lerp(Vector3.ZERO, delta * 2.0)
		else:
			_wander_t -= delta
			if _wander_t <= 0.0:
				if randf() < 0.35:
					_idle_t = randf_range(0.8, 2.0)
				else:
					_wander_t = randf_range(1.5, 3.5)
					_target_dir = Vector3(cos(randf() * TAU), 0, sin(randf() * TAU))
			_target_dir = _target_dir.normalized()

	if _mesh:
		_mesh.set_charging(_charging)

func _fire_bolt(to_player: Vector3) -> void:
	var world := get_tree().current_scene
	if world == null:
		return
	var aim := to_player.normalized()
	var muzzle := global_position + Vector3(0, 0.85, 0) + aim * 0.5
	var bolt := _ShadowBolt.new()
	world.add_child(bolt)
	bolt.global_position = muzzle
	bolt.setup(aim, attack_power, self)

func take_damage(dmg: int, attacker: Node3D = null, damage_type: int = 0) -> void:
	super.take_damage(dmg, attacker, damage_type)
	if is_alive and _mesh:
		_mesh.set_charging(false)

func _roll_exp_drop() -> void:
	var world := get_tree().current_scene
	if world == null:
		return
	if randf() < EXP_DROP_RATE * get_rate_mult():
		_ExpOrb.spawn(world, global_position, global_position.y)

func _roll_loot() -> void:
	var world := get_tree().current_scene
	if world == null:
		return
	ItemDatabase.ensure_db()
	var defn: ItemDef = ItemDatabase.items_db.get("wraith_tear")
	if defn and randf() < 0.9 * get_rate_mult():
		var count := randi_range(1, 3)
		var vel := Vector3(randf_range(-1.0, 1.0), randf_range(2.0, 3.5), randf_range(-1.0, 1.0))
		_DroppedItem.spawn(world, defn, global_position, count, vel, global_position.y)

func _die(_attacker: Node3D = null) -> void:
	_roll_loot()
	_roll_exp_drop()
	super._die(_attacker)