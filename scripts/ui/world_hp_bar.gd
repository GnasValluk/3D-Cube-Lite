...
func _build() -> void:
	...
	mat_shield.albedo_color = Color(0.20, 0.50, 1.0, 0.60)
	...
	_shield.position = Vector3(-BAR_WIDTH * 0.5, -(BAR_HEIGHT + BAR_GAP), 0.0)
...
func _process(_delta: float) -> void:
	...
	if cam == null:
		return
	var dir: Vector3 = (cam.global_position - _pivot.global_position).normalized()
	dir.y = 0.0
...
func _update_shield(current: int) -> void:
	...
	_shield.position = Vector3(-(1.0 - shield_ratio) * BAR_WIDTH * 0.5, -(BAR_HEIGHT + BAR_GAP), 0.0)
