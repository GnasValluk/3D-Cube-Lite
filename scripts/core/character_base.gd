...
# ── Stats (override trong subclass _build_character) ─────────────────────────
...
# ── Visual root (subclass phải gán trước khi return từ _build_character) ──────
var _rig: Node3D

# ── Active flag (CharacterManager dùng) ──────────────────────────────────────
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if event is InputEventKey:
		...
		if k.pressed and not k.echo:
			if k.keycode == KEY_SPACE: _jbuf = JUMP_BUFFER
			if k.keycode == KEY_F1:   _toggle_camera()
...
func _physics_process(delta: float) -> void:
	...
	if _state == State.DASH:
		...
		velocity.y  = 0.0
		if _dash_timer <= 0.0: _state = State.IDLE
		move_and_slide(); _animate(delta); return

	# Gravity ─────────────────────────────────────────────────────────────────
	if not on_floor:
		velocity.y -= (_grav_rise if velocity.y > 0.0 else _grav_fall) * delta
	else:
		velocity.y = -0.5
	...
	if _jbuf > 0.0 and _coyote > 0.0 and _state != State.CROUCH:
		velocity.y = _jump_v
		_jbuf = 0.0; _coyote = 0.0; _sy_tgt = 1.22
	...
	if _rig: _rig.scale = Vector3(1.0 + (1.0 - _sy_cur) * 0.5, _sy_cur, 1.0 + (1.0 - _sy_cur) * 0.5)
	...
	var dir := _read_input()
	if dir.length_squared() > 0.001 and not attacking:
		dir = dir.normalized()
	...
	# Dash trigger ────────────────────────────────────────────────────────────
	if Input.is_key_pressed(KEY_Q) and _dash_cd <= 0.0 and not attacking:
		var di := _read_input()
		_dash_dir = (di.normalized() if di.length_squared() > 0.001 else -global_transform.basis.z)
		_dash_dir.y = 0.0; _dash_dir = _dash_dir.normalized()
		_dash_timer = dash_duration; _dash_cd = dash_cooldown
		_state = State.DASH; _sy_tgt = 1.15
		move_and_slide(); _animate(delta); return

	# State update ────────────────────────────────────────────────────────────
	if attacking:                       _state = State.ATTACK
	elif not on_floor:                  _state = State.JUMP if velocity.y > 0.0 else State.FALL
	elif crouching:                     _state = State.CROUCH
	elif dir.length_squared() > 0.001:  _state = State.SPRINT if sprinting else State.WALK
...
func _toggle_camera() -> void:
	_use_tp = not _use_tp
	if _use_tp:
		if is_instance_valid(_iso_rig): _iso_rig.call("deactivate")
		if is_instance_valid(_tp_rig):  _tp_rig.call("activate")
	else:
		if is_instance_valid(_tp_rig):  _tp_rig.call("deactivate")
		if is_instance_valid(_iso_rig): _iso_rig.call("activate")
	await get_tree().process_frame
	_camera = get_viewport().get_camera_3d()
