...
static func build(parent: Node3D, item_id: String) -> void:
	match item_id:
		...
		"sea_helmet": _build_sea_helmet_icon(parent)
		"sea_chestplate": _build_sea_chestplate_icon(parent)
		"sea_leggings": _build_sea_leggings_icon(parent)
		"sea_boots": _build_sea_boots_icon(parent)
...
## Mũ Vỏ Sò — vỏ sò cong + dây thừng + san hô vướt lên.
static func _build_sea_helmet_icon(p: Node3D) -> void:
	var shell   := Color(0.82, 0.58, 0.40)
	var shell_h := Color(0.95, 0.72, 0.52)
	var shell_d := Color(0.62, 0.44, 0.30)
	var coral   := Color(0.92, 0.42, 0.26)
	var coral_d := Color(0.68, 0.30, 0.20)
	var rope    := Color(0.46, 0.30, 0.16)
	var pearl   := Color(0.96, 0.93, 0.88)
	ItemMeshShared.add_cube(p, 0, 0, 0, 17, 9, 17, shell)
	ItemMeshShared.add_cube(p, 0, 5, 0, 15, 5, 15, shell_h)
	ItemMeshShared.add_cube(p, 0, 10, 0, 12, 3, 12, shell)
	ItemMeshShared.add_cube(p, 0, 13, 0, 9, 3, 9, shell_h)
	ItemMeshShared.add_cube(p, 0, 16, 0, 5, 2, 5, shell_d)
	ItemMeshShared.add_cube(p, 0, -5, 0, 17.5, 2, 17.5, rope)
	ItemMeshShared.add_cube(p, 0, -4, -8, 16, 6, 3, shell_d)
	ItemMeshShared.add_cube(p, -9, -6, 5, 4, 8, 4, shell)
	ItemMeshShared.add_cube(p,  9, -6, 5, 4, 8, 4, shell)
	ItemMeshShared.add_cube(p, 0, 3, 8.5, 16, 2, 3, rope)
	ItemMeshShared.add_cube(p, 0, 1, 9, 1.5, 4, 1.5, shell_h)
	ItemMeshShared.add_cube(p, 0, -5, 8.5, 1.5, 3, 1.5, shell_d)
	ItemMeshShared.add_cube(p, 0, -7, 2, 8, 2, 2, rope)
	ItemMeshShared.add_cube(p, 0, 14.5, 0, 2.5, 3, 2.5, shell_h)
	ItemMeshShared.add_cube(p, 0, 16, 0, 1.5, 2, 1.5, pearl)
	ItemMeshShared.add_cube(p, 0, 12, -7, 2, 3, 2, coral)
	ItemMeshShared.add_cube(p, 0, 11, -9, 1.5, 2, 2, coral_d)
	ItemMeshShared.add_cube(p, 0, 10, -11, 1.5, 2, 2, coral_d)
	ItemMeshShared.add_cube(p, -7.2, 4, 0, 1.5, 1.5, 1.5, coral)
	ItemMeshShared.add_cube(p,  7.2, 4, 0, 1.5, 1.5, 1.5, coral)

## Giáp San Hô — thân xanh biển + viền san hô + dây thừng.
static func _build_sea_chestplate_icon(p: Node3D) -> void:
	var shell   := Color(0.22, 0.58, 0.54)
	var shell_h := Color(0.32, 0.70, 0.66)
	var shell_d := Color(0.14, 0.40, 0.36)
	var coral   := Color(0.90, 0.40, 0.26)
	var rope    := Color(0.46, 0.30, 0.16)
	var pearl   := Color(0.96, 0.93, 0.88)
	ItemMeshShared.add_cube(p, 0, 1, 0, 13, 10, 8, shell)
	ItemMeshShared.add_cube(p, 0, 6, 0, 9, 2, 9, shell_d)
	ItemMeshShared.add_cube(p, 0, 2, 4.5, 10, 6, 2, shell_h)
	ItemMeshShared.add_cube(p, 0, 0, 4.5, 7, 3, 1.5, shell_h.lightened(0.05))
	ItemMeshShared.add_cube(p, 0, 2, -4.5, 10, 6, 2, shell_d)
	ItemMeshShared.add_cube(p, -7.5, 3, 0, 3, 4, 3, shell_d)
	ItemMeshShared.add_cube(p,  7.5, 3, 0, 3, 4, 3, shell_d)
	ItemMeshShared.add_cube(p, 0, -3, 0, 14, 2, 6, shell_d)
	ItemMeshShared.add_cube(p, 0, -3, 4.5, 14, 0.8, 1, pearl)
	ItemMeshShared.add_cube(p, 0, -6, 0, 14, 3, 7, shell)
	ItemMeshShared.add_cube(p, 0, -9, 0, 15, 3, 8, shell)
	ItemMeshShared.add_cube(p, 0, -11, 0, 15, 1.5, 8, shell_h)
	ItemMeshShared.add_cube(p, 0, 3, 4.6, 6, 2, 0.3, shell_h.lightened(0.12))
	ItemMeshShared.add_cube(p, -7.2, 4, 0, 1.5, 1.5, 1.5, coral)
	ItemMeshShared.add_cube(p,  7.2, 4, 0, 1.5, 1.5, 1.5, coral)

## Quần Vảy Cá — vân xanh, đường kẻ dây thừng.
static func _build_sea_leggings_icon(p: Node3D) -> void:
	var shell   := Color(0.20, 0.52, 0.48)
	var shell_h := Color(0.32, 0.66, 0.62)
	var shell_d := Color(0.13, 0.36, 0.32)
	var rope    := Color(0.46, 0.30, 0.16)
	ItemMeshShared.add_cube(p, 0, 2, 0, 7, 6, 7, shell)
	ItemMeshShared.add_cube(p, 0, -1, 0, 7, 3, 7, shell_d)
	ItemMeshShared.add_cube(p, 0, -4, 0, 6, 5, 6, shell)
	ItemMeshShared.add_cube(p, 0, 1, 3, 4, 4, 1.5, shell_h)
	ItemMeshShared.add_cube(p, 0, -3, 3, 3.5, 3, 1.5, shell_h)
	ItemMeshShared.add_cube(p, 0, -6, 0, 6, 2, 6, shell_d)
	ItemMeshShared.add_cube(p, 0, 2, 0, 7, 0.6, 7, rope)
	ItemMeshShared.add_cube(p, 0, -2, 0, 7, 0.6, 7, rope)

## Giày Vây Cá — thân xanh ngầm + mũi san hô.
static func _build_sea_boots_icon(p: Node3D) -> void:
	var shell   := Color(0.22, 0.56, 0.52)
	var shell_h := Color(0.36, 0.70, 0.66)
	var shell_d := Color(0.15, 0.38, 0.34)
	var rope    := Color(0.46, 0.30, 0.16)
	var coral   := Color(0.90, 0.40, 0.26)
	ItemMeshShared.add_cube(p, 0, 3, 0, 6, 5, 6, shell_d)
	ItemMeshShared.add_cube(p, 0, 4.2, 0, 6.5, 1, 6.5, shell_h)
	ItemMeshShared.add_cube(p, 0, 0, 0, 6, 5, 7, shell)
	ItemMeshShared.add_cube(p, 0, -2, 4, 5, 3, 4, coral)
	ItemMeshShared.add_cube(p, 0, -3.5, 1, 6, 1.5, 8, shell_d)
	ItemMeshShared.add_cube(p, 0, 3.5, 0, 6, 0.8, 6, rope)
