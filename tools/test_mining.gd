...
func _ready() -> void:
	...
	_check(_D.get_block_hardness(_D.BlockID.YOUNG_GRASS) == 1.2, "cỏ non 1.2s (xẻng sắt)")
	...
	_check(not _D.is_pickaxable(_D.BlockID.YOUNG_GRASS), "cúp: không đào cỏ non")
	_check(not _D.is_pickaxable(_D.BlockID.BEDROCK), "cúp: không đào bedrock")
	_check(_D.is_shovelable(_D.BlockID.GRASS), "xẻng: cỏ")
	_check(_D.is_shovelable(_D.BlockID.YOUNG_GRASS), "xẻng: cỏ non")
